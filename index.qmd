...
### Column - Overview Conditions {width="40%"}
...
### Column - Maximum Temperatures {width="40%"}
...
MonthDistribution_plot <- function(data, from = min(data$date, na.rm = TRUE), to = max(data$date, na.rm = TRUE), 
  ...
  min_second <- ifelse(Col == 'daily_relative_humidity_2m_max', 20, 0)  # min(data$Mean, na.rm = TRUE) # Specify min of second y axis
    ...
    ggplot2::scale_y_continuous( name = 'Days', minor_breaks  = seq(0, 31, by = 1), 
                                ...
                                { if (Is_Scaled_Second_Y_Axes == TRUE) ~scale_function(., scale, shift) else ~ . * 1 },
                                  ...
                                  breaks = { if (Is_Scaled_Second_Y_Axes == TRUE) seq(0, ceiling( max(data$Mean) )
                                                  , by = ifelse(Col == 'daily_relative_humidity_2m_max', 5, 0.5) ) else seq(0, 24, by = 2) }) ) +
...
Maximum_Relative_Humidity_plot <-  function(data, from = min(data$date, na.rm = TRUE), to = max(data$date, na.rm = TRUE), 
                                   Col = 'daily_et0_fao_evapotranspiration', Mean_Col = 'daily_temperature_2m_max',
                                   Title = 'Evapotranspiration of a well watered Grass Field & Mean by Months',
                                   Bin_name = 'Evapotranspiration\nBins, mm',
                                   Mean_name = 'Evapotranspiration \nMean, mm',
  ...
  # Построение сводной столбиковой диаграммы с наложением
  ggplot2::ggplot(data_long, aes(x = Month, y = value, fill = Bin)) +
    ggplot2::geom_col(color = 'white') +
    ggplot2::geom_line(data = dplyr::distinct(data_long, Month, Mean, .keep_all = TRUE), aes(x = Month, y = { if (Is_Scaled_Second_Y_Axes == TRUE) inv_scale_function(Mean, scale, shift) else Mean }, color = 'Mean'), group = 1, linewidth = 0.7) +
    ggplot2::geom_text(data = dplyr::distinct(data_long, Month, Mean, .keep_all = TRUE), aes(label = round(Mean, 1), x = Month, y = { if (Is_Scaled_Second_Y_Axes == TRUE) inv_scale_function(Mean, scale, shift) + 1.5 else Mean + 1.5 }), size = 3, color = 'cornflowerblue') +
    
    ggplot2::scale_fill_manual( values = Bin_colors, name = 'Daily Precipitation \nBins, %' ) +
    ggplot2::scale_color_manual( values = c(Mean = 'cornflowerblue'), labels = c(Mean = 'Precipitation \nDays') ) +
    ggplot2::scale_y_continuous( name = 'Days', minor_breaks  = seq(0, 31, by = 1), 
                             sec.axis = ggplot2::sec_axis(transform = 
                            { if (Is_Scaled_Second_Y_Axes == TRUE) ~scale_function(., scale, shift) else ~ . * 1 },
                              name = Second_Y_Axes_name, 
...
Maximum_Relative_Humidity_plot(data = meteo_tbl, # , from = as.Date(params$Start_Date), to = as.Date(params$Finish_Date)
                       ...
                       Mean_name = 'Maximum \nRelative \nHumidity, %',
                       Bin_breaks = seq(20, 105, by = 5),
                       Bin_labels = c("20-25", "25-30", "30-35", "35-40", "40-45", "45-50", "50-55", "55-60", "60-65", "65-70", "70-75", "75-80", "80-85-", "85-90", "90-95", "95-99", "100") |>
      setNames( c("[20,25)", "[25,30)", "[30,35)", "[35,40)", "[40,45)", "[45,50)", "[50,55)", "[55,60)", "[60,65)", "[65,70)", "[70,75)", "[75,80)", "[80,85)", "[85,90)", "[90,95)", "[95,100)" , "[100,105)") ),
                       Bin_colors = grDevices::colorRampPalette(c('white', '#eeffff', '#d5ffff', "turquoise1", 'turquoise2' ))(17),
                       Mean_color = 'turquoise4', Second_Y_Axes_name = 'Relative Humidity, %',
...
```{r}
#| label: Distribution of Winds by Day Chart
#| fig-asp: !expr "ifelse(interactive() == TRUE, 1, 0.4)"
