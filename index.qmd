...
#| title: 'Температура воздуха по среднемесячным температурам воздуха и выпадению осадкам'
...
Union_plot <- function(data, from = min(data$date, na.rm = TRUE), to = max(data$date, na.rm = TRUE)) {
      ...
      dplyr::filter(date >= from & date <= to) %>% 
        dplyr::group_by(Month) %>%
          dplyr::summarise( high_month = base::max(daily_temperature_2m_max, na.rm = TRUE),
                            max_day   = base::max(daily_temperature_2m_mean),
                            median = stats::median(daily_temperature_2m_mean, na.rm = TRUE),
                            min_day  = base::min(daily_temperature_2m_mean),
  ...
  legend_labels <- c(
    ...
    max_day = "Maximum average \nDaily temperature",
    median = "Median average \nDaily temperature",
    min_day = "Minimum average \nDaily temperature",
    low_month = "Minimum of the \nlowest temperature \nof the Month")
  
  data %>%
    tidyr::pivot_longer(-c(Month, Precipitation), names_to = 'variable', values_to = 'value') %>%
    ggplot2::ggplot( data = . ) +
      ggplot2::geom_col( data = data, aes(x = Month, y = Precipitation), fill = 'paleturquoise' ) +
      ggplot2::geom_line(aes(x = Month, y = value / 0.0825, group = variable, color = variable, linetype = variable)) +
      ggplot2::geom_point( data = data, aes(x = Month, y = median / 0.0825), color = 'coral' ) +
      # ggplot2::geom_segment(data = data, aes(x = Month, xend = Month, y = low_month, yend = high_month, color = greenRed), size = 0.1) +
      ggplot2::geom_text(data = data, aes(label = round(median, 1), x = Month, y = median / 0.0825), size = 3, nudge_x = -0.3, nudge_y = 9, color = 'coral') +
      ggplot2::scale_color_manual( values = c(max_day = 'red', min_day = 'blue', high_month = 'red', low_month = 'blue', median = 'coral'), labels = legend_labels ) +
      ggplot2::scale_linewidth_manual( values = c(max_day = 0.1, min_day = 0.1, high_month = 1, low_month = 1, median = 2) ) +
      ggplot2::scale_linetype_manual(values = c(max_day = 'dashed', min_day = 'dashed', high_month = 'dotted', low_month = 'dotted', median = 'solid'), labels = legend_labels ) +
      ggplot2::scale_y_continuous(name = 'Average Month Precipitation, mm', sec.axis = ggplot2::sec_axis(transform = ~ . * 0.0825, name = 'Air Temperature, °C')) +
      ggplot2::labs(title = Title, color = 'Variable', linetype = 'Time Line') +
      ggplot2::theme_minimal() +
      # ggplot2::lims(y = c(16, NA)) +
      ...
      ggplot2::guides(color = guide_legend(title = ''), linetype = guide_legend(title = ''))
...
# Data & Map {orientation="rows" scrolling="true"}

## Row - Описание сессии {height="80%"}

::: {.card title='Рассмотрим исторические данные' width="70%"}

### Column - Interaction Table {width="70%"}
...
### Column - Interaction Map {width="30%"}
