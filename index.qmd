...
MonthDistribution_plot <- function(data, from = min(data$date, na.rm = TRUE), to = max(data$date, na.rm = TRUE), 
  ...
  data <- 
    ...
    #   janitor::tabyl(Month, !!rlang::sym(Col))
    #   } else { 
    # Группы по Градациям Переменной по дням Месяцев
        dplyr::mutate( Bin = factor(base::cut(!!rlang::sym(Col), breaks = Bin_breaks, right = FALSE)) ) |>
    ...
    dplyr::inner_join(
      y = { if (Mean_Col == 'daily_temperature_2m_max') {
        # Среднемесячное число дней  заморозки
        dplyr::mutate(data, Freezing_Days = dplyr::if_else(!!rlang::sym(Mean_Col) < 0, 1, 0) ) |>
                dplyr::group_by(Month) |> 
                  dplyr::summarise(Mean = mean(Freezing_Days, na.rm = TRUE )) |>
                    dplyr::left_join(MonthDays_tbl, by = dplyr::join_by(Month == Month)) |>
                      dplyr::mutate(dplyr::across(dplyr::where(is.numeric), ~ . * Days)) |>
                        dplyr::select(-Days)
      } else { if (Mean_Col == 'cloud_cover') {
        ...
        # Среднедневная сумма Переменной (Обобщенный Случай)
         dplyr::group_by(data, Month) |> 
           dplyr::summarise(Mean = mean(!!rlang::sym(Mean_Col), na.rm = TRUE)) }
        }   # The End of if (Mean_Col == 'cloud_cover')
  ...
  data_long <- 
    tidyr::pivot_longer( data,
      ...
      dplyr::mutate( Month = factor(Month, levels = month.abb)
                   ...
                   , Bin = factor(Bin, levels = rev(names(data)[c(-1, -ncol(data))]),
                                  labels = Bin_labels[ rev(names(data)[c(-1, -ncol(data))]) ]) )
  
  # Automatically defining the scaling function for Two axes of Y - https://finchstudio.io/blog/ggplot-dual-y-axes/
  max_first  <- max(data_long$value, na.rm = TRUE) # Specify max of first y axis
  ...
  min_second <- 0  # min(data$Mean, na.rm = TRUE) # Specify min of second y axis
  ...
  p <- ggplot2::ggplot(data_long, aes(x = Month, y = value, fill = Bin)) +
    ...
    ggplot2::scale_y_continuous( name = 'Days', minor_breaks  = seq(0, 31, by = 1), 
                                 sec.axis = ggplot2::sec_axis(transform = { if (Is_Scaled_Second_Y_Axes == TRUE) ~scale_function(., scale, shift) else ~ . * 1 }, name = Second_Y_Axes_name, breaks = { if (Is_Scaled_Second_Y_Axes == TRUE) seq(0, ceiling( max(data$Mean) ), by = 0.5) else seq(0, 24, by = 2) }) ) +
...
Cloudy_Sunny_Precipitation_plot <- function(data, from = min(data$date, na.rm = TRUE), to = max(data$date, na.rm = TRUE)) {
  ...
  ggplot2::ggplot(data_long, aes(x = Month, y = value, fill = Cloudiness)) +
    ggplot2::geom_col(color = 'white') +
    ggplot2::geom_line(data = dplyr::distinct(data_long, Month, Precipitation_Days, .keep_all = TRUE), aes(x = Month, y = Precipitation_Days, color = 'Precipitation_Days'), group = 1, linewidth = 0.7) +
    ...
    ggplot2::scale_y_continuous(minor_breaks  = seq(0, 31, by = 1)) +
...
```{r}
#| label: Distribution of Relative Humidity by Day Chart
#| fig-asp: !expr "ifelse(interactive() == TRUE, 1, 0.4)"
