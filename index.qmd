...
subtitle: "using {openmeteo} package with Quarto dashboard by"
---
...
IsSnowCover <- max(meteo_tbl$daily_snow_depth_max) >= 0.03
...
Union_plot <- function(data, from = min(data$date, na.rm = TRUE), to = max(data$date, na.rm = TRUE)) {
  ...
  ggplot2::ggplot( data = data_long ) +
    ...
    ggplot2::scale_fill_manual( values = c(Precipitation = '#a9c4f5'), labels=c(Precipitation = 'Mean Precipitation, mm') ) +
...
MonthDistribution_plot <- function(data, from = min(data$date, na.rm = TRUE), to = max(data$date, na.rm = TRUE), 
  ...
  if (Col == 'daily_snow_depth_max') {
    # Если есть, перемещаем ее в конец
    data <- data |>
      dplyr::select(-tidyselect::one_of("[-Inf,0.5)"))
  ...
  data_long <- 
    ...
    dplyr::mutate( Month = factor(Month, levels = month.abb)
                 , Mean = if (Mean_Col %in% c('daily_temperature_2m_max', 'daily_temperature_2m_min', 'daily_snow_depth_max')) dplyr::if_else(Mean < 0.03, NA, Mean) else Mean
  ...
  # Automatically defining the scaling function for Two axes of Y - https://finchstudio.io/blog/ggplot-dual-y-axes/
  max_first  <- ifelse(Col %in% c('daily_snow_depth_max', 'daily_relative_humidity_2m_mean', 'daily_relative_humidity_2m_max', 'daily_relative_humidity_2m_min', 'daily_wind_speed_10m_max'), 31, max(data_long$value, na.rm = TRUE)) # Specify max of first y axis
  ...
  min_second <- ifelse(Col %in% c('daily_snow_depth_max', 'daily_relative_humidity_2m_mean', 'daily_relative_humidity_2m_max', 'daily_relative_humidity_2m_min', 'daily_wind_speed_10m_max'),  nearest_multiple_of_5_down_function(min(data$Mean, na.rm = TRUE)), 0) # Specify min of second y axis
    ...
    ggplot2::scale_y_continuous( name = 'Days', minor_breaks  = seq(0, 31, by = 1), 
                                ...
                                { if (Is_Scaled_Second_Y_Axes == TRUE) ~scale_function(., scale, shift) else ~ . * 1 },
                                  ...
                                  breaks = { if (Is_Scaled_Second_Y_Axes == TRUE) seq(0, ceiling( max(data$Mean, na.rm = TRUE) )
                                                  , by = ifelse(Col %in% c('daily_snow_depth_max', 'daily_relative_humidity_2m_mean', 'daily_relative_humidity_2m_max', 'daily_relative_humidity_2m_min', 'daily_wind_speed_10m_max'), 5, 0.5) ) else seq(0, 24, by = 2) }) ) +
...
MonthDistribution_plot(data = meteo_tbl, # , from = as.Date(params$Start_Date), to = as.Date(params$Finish_Date)
                       ...
                       Title = 'Maximum Daily Snow Depth & Mean Maximum Show Depth by Months',
                       Bin_name = 'Maximum Daily Snow Depth \nBins, cm',
                       Mean_name = 'Mean Maximum Daily Snow Depth, \ncm',
    ...
    setNames( c("[-Inf,0.5)", "[0.5,1)", "[1,5)", "[5,10)", "[10,20)", "[20,40)", "[40,60)", "[60,80)", "[80,100)", "[100, Inf)") ),
                       Bin_colors = grDevices::colorRampPalette(c("white", "cyan", "#007FFF", "blue", "#00007F"))(10),
                       Mean_color = 'gold3', Second_Y_Axes_name = 'Cm', Is_Scaled_Second_Y_Axes = TRUE)

```
`r if (IsSnowCover == TRUE) ":::"`

`r if (IsSnowCover == TRUE) '#### Row - Relative Humidity  {.tabset heigth="54%"}' else '#### Row - Relative Humidity  {.tabset}'`
