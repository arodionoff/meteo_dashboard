  ...
  dplyr::inner_join(
    ...
    y = dplyr::mutate( meteo_hourly_df # Month = factor(lubridate::month(datetime), labels = month.abb)
          ...
          #   dplyr::rename_with(~ gsub('^hourly_', 'daily_', .), dplyr::starts_with('hourly_'))
            dplyr::summarise( cloud_cover = dplyr::if_else( mean(cloud_cover) == 1, 'Sunny', dplyr::if_else( mean(cloud_cover) == 3, 'Overcast', 'Cloudy'))
...
remove(meteo_daily_df, WCO_Codes_tbl, ONI_tbl)
...
```{r}
#| label: Union Air Temperature & Precipitation Chart First
#| title: 'Diagram of Monthly Air Temperatures and Precipitation'
...
Union_plot <- function(data, from = min(data$date, na.rm = TRUE), to = max(data$date, na.rm = TRUE)) {
  ...
  ggplot2::ggplot( data = data_long ) +
    ...
    ggplot2::scale_y_continuous(name = 'Average Month Precipitation, mm', limits = c(min_first, max_first), breaks = seq(0, 2000, by = ifelse(max_first - min_first > 200, 50, 25)), sec.axis = ggplot2::sec_axis(~scale_function(., scale, shift), breaks = seq(-40, 40, by = ifelse(max_second - min_second > 30, 5, 2)), name = 'Air Temperature, °C')) +
    ggplot2::labs(title = Title) + # , color = 'Color', fill = 'Fill', shape = 'Shape', linetype = 'Line Type') +
...
#### Row - Diagram of Monthly Air Temperatures and Precipitation {height="100%"}

```{r}
#| label: Cloudy, Sunny and Precipitation Days Chart
#| title: 'Diagram of Cloudy, Sunny and Precipitation Days'
#| fig-asp: !expr "ifelse(interactive() == TRUE, 1, 0.4)"

# Функция для создания графика по дням, когда было Облачно, Солнечно и Дням осадков
Cloudy_Sunny_Precipitation_Day_plot <- function(data, from = min(data$date, na.rm = TRUE), to = max(data$date, na.rm = TRUE)) {
  ...
  data_long <- 
    tidyr::pivot_longer( data,
                         cols = c(Sunny, Cloudy, Overcast),
                         names_to = 'Cloudiness',
                         values_to = 'value' ) |>
      dplyr::mutate( Month = factor(Month, labels = month.abb)
                   , Precipitation_Days = NULL
                   , Cloudiness = factor(Cloudiness, levels = c('Overcast', 'Cloudy', 'Sunny')) )

  legend_colors <- c(Sunny = 'yellow', Cloudy = '#dde7fb', Overcast = 'cornflowerblue')

  # Построение сводной столбиковой диаграммы с наложением
  ggplot2::ggplot(data_long, aes(x = Month, y = value, fill = Cloudiness)) +
    ggplot2::geom_bar(stat = 'identity') +
    ggplot2::scale_fill_manual( values = legend_colors ) +
    ggplot2::labs(title = Title, y = 'Days', fill = 'Cloudiness') +
    ggplot2::theme_minimal() +
    # ggplot2::coord_cartesian(ylim = c(15, NA)) +
    ggplot2::theme( plot.title = ggplot2::element_text(hjust = 0.5),
                    legend.position = 'right', legend.text = element_text(size = 7),
...
}    # The End of function `Cloudy_Sunny_Precipitation_Day_plot()`

# Пример использования функции
Cloudy_Sunny_Precipitation_Day_plot(data = meteo_tbl) # , from = as.Date(params$Start_Date), to = as.Date(params$Finish_Date)
...
meteo_tbl |>
  ...
  DT::datatable(options = list(dom = 'Bfrtip', paging = TRUE) )  # buttons = c('copy', 'csv', 'excel'), scrollX = TRUE,     fixedColumns = list(leftColumns = 2, rightColumns = 1), 
